...
- Current embedded Ghostty panes expose the local PTY foreground process group and TTY name while explicitly reporting what the backend still cannot prove. Authoritative foreground command text, alternate-screen state, and remote-host identity remain unavailable, so manual tmux/editor/SSH flows may stay `unknown`.
